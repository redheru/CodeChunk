...
//
	...
	bool ReadHierarchy(ifstream& file, Skeleton* skl);
	bool ReadMotionInfo(ifstream& file);
	bool ReadMotionData(ifstream& file);

	string getChannelNameString( klVec3 eulerOrder, bool isPosition);

	void WriteHierarchy( Bone* bone, ofstream& ofs, int depth);
...
// >getter
	...
	int getBoneNum(){return (int)skeleton_->bones.size();}
	int getFrameNum(){ return frameNum_; }
	bool getIsLoaded(){return loaded_;}
	//set by AddPose()
	//void setFrameNum(int frame_num){ this->frameNum = frame_num; }
	klVec3 getBoneDirection(int bone_index){ return skeleton_->bones[bone_index]->direction; }
	string getName(){ return name_; }
	double getFrameTime(){ return frameTime_; }
	int getBoneIndex(string boneName){return skeleton_->getBoneIndex(boneName);}
	string getBoneName(int boneIndex){return skeleton_->getBoneName(boneIndex);}
	//fastest acces
	...
	{
		int index = skeleton_->getBoneIndex(boneName);
...
// > setter
	...
	bool SetBoneAngle(klQuat q, int frame, string bone_name){return SetBoneAngle(q, frame,  skeleton_->getBoneIndex(bone_name));}
