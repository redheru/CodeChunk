...
namespace GLUT
{
	...
	{
	private:
		int texWidth_;               // テクスチャ横サイズ(シャドウマップ)
	    int texHeight_;              // テクスチャ縦サイズ(シャドウマップ) 
        ...
        GLuint vertShader_;          // バーテックスシェーダオブジェクト
        GLuint fragShader_;          // フラグメントシェーダオブジェクト
        GLuint gl2Program_;

        GLuint tex_;                 // テクスチャオブジェクト
        GLuint fb_;                  // フレームバッファオブジェクト
	...
	private:
		void Init();
		void createShadowMap(void);          // 床井さんの実装を参考に作成。3パスでのシャドウマップを実現(シェーダを使ったので2パスになってる)
