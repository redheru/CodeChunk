...
Skeleton* MocapData::ReadHierarchy(ifstream& file)
{
	Skeleton* skl = new Skeleton();
	if(skl->BuildSkeleton(file) == false) return NULL;
	return skl;
}
...
bool MocapData::ReadMotionData(ifstream& file, Skeleton* skl)
{
	#define  BUFFER_LENGTH  1024*4
	char      line[ BUFFER_LENGTH ];
	char *    token;
	...
	int channelNum = skl->getDataSize();//RootPos + BoneRot
...
bool MocapData::LoadMotion(string fileName)
{
	...
	Skeleton* skl = ReadHierarchy(file);
	...
	//回転角度などを読み込み
	if(ReadMotionData(file, skl) == false) return false;
...
void MocapData::WriteHierarchy(Skeleton* skl, Bone* bone, ofstream& ofs, int depth)
{
	string indent;
	...
	if(bone->children.size() == 0)
	{
		...
		ofs << indent << "\t" << "OFFSET";
		// todo: 出力桁数の整形
		klVec3 offset = bone->direction * bone->length;
	...
	// ここからインデントがさらに1段下がる
	{
		...
		{ //Rootの場合の処理(Position -> Rotationの順に出力する)
			int rootChannelNum = bone->eulerOrder.size() + skl->positionEulerOrder.size();
			ofs << indent2 << "CHANNELS "
				<< rootChannelNum;
			for(unsigned int i = 0; i < skl->positionEulerOrder.size(); i++) 
				ofs << " " << getChannelNameString( skl->positionEulerOrder[i], true);
		...
		{
			WriteHierarchy( skl, skl->bones[bone->children[i]], ofs, depth );
		}
...
{
	if(this->poses.size() <= 0) return false;
	Skeleton* skl = this->poses[0].getSkeleton();
	...
	WriteHierarchy( skl, skl->bones[0], ofs, 0);
	...
	for( int frame = 0; frame < frameNum_; frame++ )
	{
		...
		// rootの情報
		//// rootの位置
		klVec3 position = pose.getRootPosition();
		for(int i = 0; i < 3; i++)
		{
			if(skl->positionEulerOrder[i].x >= 1.0) ofs << position.x << " ";
			else if(skl->positionEulerOrder[i].y >= 1.0) ofs << position.y << " ";
			else if(skl->positionEulerOrder[i].z >= 1.0) ofs << position.z << " ";
			else return false;
		}
		//// rootの向き
		...
		for( int i = 1; i < skl->getBoneNum(); i++ )
		{
			if(skl->bones[i]->children.size() > 0)
			{//EndSiteではない
				int index = skl->bones[i]->children[0];
